...
for f in fileList:
    ...
    suffix =fname_splitted[-1].replace('.nc','')[1:]
    ds=xr.open_dataset(f)
    buffer.append(ds)
    years.append(year)

from_year=natsorted(list(set(years)))[0][:4]
to_year=natsorted(list(set(years)))[-1][:4]

outname=f'{exp}_{from_year}-{to_year}_{suffix}'
