...
parser.add_argument('-d', '--decim_factor', help='decimation factor')
...
ds = dataset.read()
lr_decim = ds.isel(longitude=np.arange(0, len(ds.longitude.values),args.decim_factor),latitude=np.arange(0, len(ds.latitude.values),args.decim_factor))
...
if args.output:
    ...
    lr_decim.to_netcdf(os.path.join(base,f"{name}"))